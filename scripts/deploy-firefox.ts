...
function getCredentials(): { issuer: string; secret: string; id?: string } {
  ...
  const id =
    process.env.AMO_EXTENSION_ID ||
    "screen-recorder-pro@devlopersabbir.github.io";
...
async function main() {
  console.log(
    "🚀 [Screen Recorder Pro] Preparing Firefox Add-on for Mozilla Developer Hub (AMO)..."
  );
  ...
  spawnSync(
    "git",
    ["archive", "--format=zip", "--output=sources.zip", "HEAD"],
    {
      stdio: "inherit",
      shell: true,
    }
  ...
  const approvalNotes = [
    "Screen Recorder Pro is an open-source WebExtension built with TypeScript, React, and Vite.",
    "",
