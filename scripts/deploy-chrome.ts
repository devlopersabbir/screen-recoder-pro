...
async function main() {
  console.log(
    "🚀 [Screen Recorder Pro] Preparing Chrome Extension for Chrome Web Store..."
  );
  ...
  const publishTarget = (targetArg ||
    process.env.CHROME_PUBLISH_TARGET ||
    "default") as "default" | "trustedTesters";

  const { extensionId, publisherId, clientId, clientSecret, refreshToken } =
    getCredentials();
  ...
  console.log(
    `📤 Uploading ${zipPath} to Chrome Web Store (Extension ID: ${extensionId}, Publisher ID: ${publisherId})...`
  );
  ...
  try {
    ...
    if (shouldPublish) {
      console.log(
        `📢 Submitting item for review / publishing (target: ${publishTarget})...`
      );
      ...
      console.log(
        "ℹ️ Extension uploaded as draft. To publish immediately, pass --publish or set CHROME_AUTO_PUBLISH=true."
      );
    }

    console.log(
      `🔗 Dashboard: https://chrome.google.com/webstore/devconsole/${publisherId}/${extensionId}`
    );
  } catch (err: any) {
